...
if [ -f "target/ios/aarch64-apple-ios/release/libtor.a" ] && \
   [ -f "target/ios/x86_64-apple-ios/release/libtor.a" ]; then
    mkdir -p target/ios/universal
    lipo -create \
        target/ios/aarch64-apple-ios/release/libtor.a \
        target/ios/x86_64-apple-ios/release/libtor.a \
        -output target/ios/universal/libtor.a
    echo "Universal static library created at target/ios/universal/libtor.a"
else
